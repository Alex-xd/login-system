/**
 * @fileoverview The sign in verification page component.
 */
import React, { useState, useEffect } from "react";
import { useNavigate } from "react-router-dom";
import { Button, Typography, message } from "antd";
import { sendLoginCode, verifyLogin } from "../services/authService";
...
const VerifyLogin: React.FC = () => {
  ...
  const handleVerifyLogin = async (code: string) => {
    ...
    try {
      ...
      localStorage.setItem("token", data.data.data); // save the token, for the next page
  ...
  return (
    <AuthLayout>
      ...
      <a onClick={() => navigate("/signin")} className={styles.goBack}>
        Go back
      </a>
    </AuthLayout>
  );
